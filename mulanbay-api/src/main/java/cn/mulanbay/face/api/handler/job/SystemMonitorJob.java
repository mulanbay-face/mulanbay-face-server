package cn.mulanbay.face.api.handler.job;
...
import cn.mulanbay.face.api.handler.PmsScheduleHandler;
...
public class SystemMonitorJob extends AbstractBaseJob {
    ...
    public TaskResult doTask() {
        ...
        if (diskRate >= para.getDiskMaxRate()) {
            ...
            pmsNotifyHandler.addMessageToNotifier(ApiErrorCode.DISK_ALERT, "磁盘报警", content, new Date(), null);
        ...
        if (memoryRate >= para.getMemoryMaxRate()) {
            ...
            pmsNotifyHandler.addMessageToNotifier(ApiErrorCode.MEMORY_ALERT, "内存报警", content, new Date(), null);
        ...
        if (cpuRate >= para.getCpuMaxRate()) {
            ...
            pmsNotifyHandler.addMessageToNotifier(ApiErrorCode.CPU_ALERT, "CPU报警", content, new Date(), null);
    ...
    private void doAlertAutoJobs(String jobs) {
        ...
        for (String s : ids) {
            try {
                ...
                pmsNotifyHandler.addMessageToNotifier(ApiErrorCode.SYSTEM_ALERT_AUTO_JOB, "系统报警后执行自动执行任务", content, new Date(), "系统报警后执行自动执行任务");
            } catch (Exception e) {
