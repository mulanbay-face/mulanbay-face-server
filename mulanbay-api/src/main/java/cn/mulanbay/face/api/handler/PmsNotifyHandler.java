...
import cn.mulanbay.business.domain.SystemMonitorUser;
import cn.mulanbay.business.domain.UserMessage;
import cn.mulanbay.business.enums.LogLevel;
import cn.mulanbay.business.enums.MessageSendStatus;
import cn.mulanbay.business.enums.MessageType;
...
import cn.mulanbay.face.api.handler.msg.RedisDelayQueueHandler;
    ...
    public Long addNotifyMessage(int code, String title, String content, Long userId, Date notifyTime, String url) {
        ...
        if (ec == null) {
            logHandler.addSystemLog(LogLevel.WARNING, "错误代码未配置", "代码[" + code + "]没有配置",
                    ApiErrorCode.ERROR_CODE_NOT_DEFINED);
            return null;
        }
        ...
        try {
            ...
            if (ec == null) {
                logHandler.addSystemLog(LogLevel.WARNING, "错误代码未配置", "代码[" + code + "]没有配置,系统采用通用提醒代码配置",
                        ApiErrorCode.ERROR_CODE_NOT_DEFINED);
                ec = systemConfigHandler.getErrorCodeDefine(ApiErrorCode.MESSAGE_NOTIFY_COMMON_CODE);
            }
            this.updateErrorCodeCount(code, 1);
            //获取发送时间
            Date expectSendTime = this.getExpectSendTime(ec, notifyTime);
